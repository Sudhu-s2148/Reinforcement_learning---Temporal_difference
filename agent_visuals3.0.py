...
def draw_arrow(surface, rect, action_idx, cell_color):
    """Small direction arrow inside a cell.
    Adjust dirs to match your state_updater action mapping:
    ...
    tip     = (cx + dx * TILE_SIZE // 5, cy + dy * TILE_SIZE // 5)
    base    = (cx - dx * TILE_SIZE // 7, cy - dy * TILE_SIZE // 7)
    color   = lerp_color(cell_color, (255, 255, 255), 0.55)
    pygame.draw.line(surface, color, base, tip, 2)
    px, py  = -dy, dx
    h1      = (tip[0] - dx * 5 + px * 4, tip[1] - dy * 5 + py * 4)
    h2      = (tip[0] - dx * 5 - px * 4, tip[1] - dy * 5 - py * 4)
...
def draw_frame(screen, static_surf, maze_R, maze_Q, agent_pos, episode_num,
    ...
    hint = "H: normal" if heatmap_on else "H: heatmap"
    screen.blit(font_small.render(hint, True, HUD_DIM), (cols * TILE_SIZE // 2 + 60, 22))

    # 7. heatmap legend strip
    if heatmap_on:
        leg_x = 8
        leg_y = rows * TILE_SIZE + HUD_H - 20
        leg_w = 140
        leg_h = 8
        ...
        screen.blit(lo_lbl, (leg_x, leg_y - 13))
        screen.blit(hi_lbl, (leg_x + leg_w - hi_lbl.get_width(), leg_y - 13))
...
def main():
    ...
    while running and episode < TOTAL_EPISODES:
        ...
        while state != goal and moves < TOTAL_MOVES :
            ...
            if show_this_one:
                tick += 1
                static_surf = heatmap_surf if heatmap_on else normal_surf
                draw_frame(screen, static_surf, maze_R, maze_Q, state,
        ...
        # rebuild heatmap surface once per episode (Q-values have changed)
        if heatmap_on or (episode % EPISODE_STEP == 0):
            heatmap_surf = build_heatmap_surface(maze_R, maze_Q, goal, cols, rows)
            q_min, q_max = compute_q_range(maze_Q, maze_R)
        if state == goal:
